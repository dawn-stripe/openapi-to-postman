  ...
  * @param {object} schemaCache - object storing schemaFaker and schmeResolution caches
  ...
  convertToPmBody: function(requestBody, requestType, components, options, schemaCache) {
    ...
    if (contentObj.hasOwnProperty(URLENCODED)) {
      ...
      schema_requirements = _.get(contentObj[URLENCODED], ['schema', 'required'], {});
      ...
      _.forOwn(bodyData, (value, key) => {
        ...
        if (_.get(contentObj[URLENCODED], 'schema.type') === 'object') {
          ...
          //whether the required property is set on the parameter
          //whether the parameter is included in the required section of the request's schema.
          required_on_param = _.get(contentObj[URLENCODED], ['schema', 'properties', key, 'properties', 'required'], false);
          required_by_schema = _.includes(schema_requirements, key);
          required = (required_on_param || required_by_schema);
        ...
        encoding[key].required = required;
      ...
      //todo: refactor to use logic for required parameters above.
      rDataMode = 'formdata';
